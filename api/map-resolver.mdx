---
title: "MapResolver API Reference"
sidebarTitle: "MapResolver"
description: "Full API reference for MapResolver — mapres's core resolution engine. Documents the constructor, res() method, pipeline, syntax providers, and the global res() shortcut."
---
...
<ParamField path="layers" type="list | None" default="LayerStack()">
  A `LayerStack` instance or a list of `Layer` objects that define the ordered
  map sources used during resolution. When `None` or omitted, an empty
  `LayerStack` is used and no map lookups are performed.
</ParamField>

---
title: "DataMaps: Typed Template Contexts for mapres"
...
<Note>
`providers` itself is not emitted into the resolved map — it does not appear as a key in the output of `as_map()` because it is not a regular init field tracked by `fields()`.
</Note>
